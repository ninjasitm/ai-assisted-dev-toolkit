...
5.5. **Update Claude Code Rule Scoping**:

After replacing placeholders, update the `paths:` frontmatter in `.claude/rules/*.md` files to match the actual project directory structure. Rules use YAML `paths:` frontmatter to scope when they load — they only activate when Claude works with files matching those glob patterns. Rules without `paths:` load unconditionally at session start.

**Review each path-scoped rule and adjust paths to match your project:**

| Rule File                                | Default Paths                                      | Adjust To Match                              |
| ---------------------------------------- | -------------------------------------------------- | -------------------------------------------- |
| `api-server.md`                          | `src/server/**`, `src/api/**`, `**/*.server.*`     | Your API/server directories                  |
| `composition-api-component-structure.md` | `src/components/**`, `**/*.{vue,tsx,jsx}`          | Your component directories and extensions    |
| `core-libraries.md`                      | `src/**`                                           | Your source directory                        |
| `logging.md`                             | `src/**`                                           | Your source directory                        |
| `logging-comments.md`                    | `src/**`                                           | Your source directory                        |
| `prisma-migrations.md`                   | `prisma/**`, `**/migrations/**`, `**/*.prisma`     | Your ORM/migration directories               |
| `server-client-separation.md`            | `src/**`                                           | Your source directory                        |
| `testing-quality.md`                     | `**/*.{test,spec}.*`, `tests/**`, `__tests__/**`   | Your test directories and naming conventions |
| `ui-accessibility.md`                    | `src/components/**`, `**/*.{vue,tsx,jsx,css,scss}` | Your UI/component directories                |

**Common adjustments:**

- Next.js: Change `src/server/**` → `app/api/**`, add `**/*.server.{ts,tsx}`
- Laravel: Change `src/**` → `app/**`, `resources/**`
- Django: Change `src/**` → `{{APP_NAME}}/**`
- .NET: Change `src/**` → `Controllers/**`, `Services/**`
- Rails: Change `src/components/**` → `app/views/**`, `app/helpers/**`
...
**Remove rules that don't apply** to your project (e.g., remove `prisma-migrations.md` if you don't use Prisma, remove `composition-api-component-structure.md` if you don't have a component framework).
...
8. **Recommend and Install AI Agent Skills**:
   ...
   **Example commands** (replace `<detected-agents>` with the actual flags from step 7, e.g., `-a cursor -a github-copilot`):
   ...
   **Skill Creation for Unsupported Frameworks:**
   ...
   Use `npx -y skills add <detected-agents> anthropics/skills --skill '*' --agent github-copilot cursor` (includes `skill-creator`) to create custom skills for frameworks not yet in the ecosystem.
   ...
   **Note:** Skills are installed by skill name, not org/repo path. For example, `npx -y skills add obra/superpowers --skill '*' --agent github-copilot cursor` installs to `.cursor/skills/superpowers/`.
