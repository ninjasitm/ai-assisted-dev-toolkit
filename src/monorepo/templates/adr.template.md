# ADR-{{ADR_NUMBER}}: {{DECISION_TITLE}}

**Status:** Proposed | Accepted | Deprecated | Superseded
**Date:** {{DATE}}
**Author:** {{AUTHOR}}
**Deciders:** {{DECIDERS}}
**Technical Story:** {{ISSUE_ID}}

## Context

Describe the context and problem statement. What is the issue that we're seeing that is motivating this decision or change?

### Background

Provide any relevant background information:

- Current state of the system
- Technical constraints
- Business requirements
- Timeline pressures

### Problem Statement

In 1-2 sentences, what problem are we trying to solve?

## Decision Drivers

What factors are important in making this decision?

- **Driver 1:** Description
- **Driver 2:** Description
- **Driver 3:** Description
- **Driver 4:** Description

## Considered Options

### Option 1: {{OPTION_1_NAME}}

**Description:** Brief description of the approach.

**Pros:**
- Pro 1
- Pro 2

**Cons:**
- Con 1
- Con 2

**Estimated Effort:** X days/weeks

### Option 2: {{OPTION_2_NAME}}

**Description:** Brief description of the approach.

**Pros:**
- Pro 1
- Pro 2

**Cons:**
- Con 1
- Con 2

**Estimated Effort:** X days/weeks

### Option 3: {{OPTION_3_NAME}}

**Description:** Brief description of the approach.

**Pros:**
- Pro 1
- Pro 2

**Cons:**
- Con 1
- Con 2

**Estimated Effort:** X days/weeks

## Decision Outcome

### Chosen Option

**Option X: {{CHOSEN_OPTION_NAME}}**

### Rationale

Explain why this option was chosen:

- Reason 1
- Reason 2
- Reason 3

### Trade-offs Accepted

What are we giving up by choosing this option?

- Trade-off 1
- Trade-off 2

## Consequences

### Positive

- Positive consequence 1
- Positive consequence 2

### Negative

- Negative consequence 1
- Negative consequence 2

### Neutral

- Neutral consequence 1

## Implementation

### Action Items

- [ ] Action item 1
- [ ] Action item 2
- [ ] Action item 3

### Migration Plan

If applicable, describe how to migrate from current state to new state.

### Rollback Plan

If applicable, describe how to rollback if needed.

## Validation

How will we know if this decision was successful?

- Success metric 1
- Success metric 2

## Related Decisions

- [ADR-XXX](./ADR-XXX-title.md) - Related decision
- [ADR-YYY](./ADR-YYY-title.md) - Superseded by this ADR

## References

- [Link to relevant documentation]
- [Link to technical specification]
- [Link to discussion thread]

---

## Changelog

| Date       | Change                              |
| ---------- | ----------------------------------- |
| {{DATE}}   | Initial proposal                    |
